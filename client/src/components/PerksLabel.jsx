            ...
            <div className="grid gap-2 grid-cols-3 md:grid-cols-3 lg:grid-cols-4">
                                {
                                    perksList.map(perk =>
                                        <>
                                            <label htmlFor={perk} className="flex gap-4 mt-2 border p-2 rounded-lg cursor-pointer" role={perk}>
                                                <input type="checkbox" checked={checkedList.includes('Wifi')} name="wifi" id="wifi" onChange={() => handleCbChange(perk)}/>
